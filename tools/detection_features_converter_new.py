...
def main():
    ...
    parser.add_argument(
        ...
        choices=["slake", "slake-cp", "vqa-rad", "vqa-rad-cp", "pathvqa", "vqavs", "vqacp-v2", "vqace", "gqaood", "pmc", "pmc-cp", "pmc2", "pmc2-cp", "pmca", "pmca-cp"],
        help='choose dataset'
    )
    ...
    with open(bottom_up_path, 'r') as tsv_in_file:
        ...
        for item in tqdm(reader, total=num_images):
            ...
            if dataset == 'vqacp-v2' or dataset == 'vqace' or dataset == 'vqavs':
                image_id = int(image_id.split('_')[2].lstrip('0'))
            if dataset == 'gqaood' and image_id.startswith('n'):
                image_id = image_id[1:]
