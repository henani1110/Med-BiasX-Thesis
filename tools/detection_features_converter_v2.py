...
def main():
    ...
    parser.add_argument(
        ...
        choices=["slake", "slake-cp", "vqa-rad", "vqa-rad-cp", "vqacp-v1", "vqacp-v2", "vqa-v2", "vqace", "gqaood"],
        help='choose dataset'
    )
    ...
    for split in split_set:
        ...
        if os.path.exists(split_ids_path):
            print(len(json.load(open(split_ids_path, 'r'))))
    ...
    with open(bottom_up_path, 'r') as tsv_in_file:
        ...
        for item in tqdm(reader, total=num_images):
            image_id = int(item['img_id'].split('_')[2].lstrip('0'))
